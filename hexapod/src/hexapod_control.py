...
class NN_PG_SB(nn.Module):
    def __init__(self, hid_dim=96, obs_dim=None, act_dim=None):
...
class AHRS:
    ...
    def update(self, heading_spoof_angle=0):
        with self.lock:
            ...
            return self.roll, self.pitch, yaw_corrected, self.quat, self.timestamp
...
class AHRS_RS:
    ...
    def update(self, heading_spoof_angle=0):
        ...
        return roll, pitch, yaw_corrected, quat_yaw_corrected, self.timestamp
...
class HexapodController:
    def __init__(self, config):
        ...
        self.joints_rads_low = np.array([-0.5, -0.9, 0.2] * 6)
        self.joints_rads_high = np.array([0.5, 0.7, 1.4] * 6)
        ...
        obs_dim = 23 + 6 * self.config["use_contacts"]

        logging.info("Loading policies: ")
        self.nn_policy_straight = NN_PG_SB(hid_dim=self.config["policy_hid_dim"], obs_dim=obs_dim, act_dim=18)
        self.nn_policy_straight_rough = NN_PG_SB(hid_dim=self.config["policy_hid_dim"], obs_dim=obs_dim, act_dim=18)
        self.nn_policy_turn_left = NN_PG_SB(hid_dim=96, obs_dim=obs_dim, act_dim=18)
        self.nn_policy_turn_right = NN_PG_SB(hid_dim=96, obs_dim=obs_dim, act_dim=18)

        self.nn_policy_straight.load_state_dict(T.load(self.config["policy_straight"]))
        self.nn_policy_straight_rough.load_state_dict(T.load(self.config["policy_straight_rough"]))
        self.nn_policy_turn_left.load_state_dict(T.load(self.config["policy_turn_left"]))
        self.nn_policy_turn_right.load_state_dict(T.load(self.config["policy_turn_right"]))
    ...
    def start_ctrl_loop(self):
        ...
        while True:
            ...
            if not self.idling:
                ...
                policy_act = self.current_nn_policy(policy_obs)[0].detach().numpy()
    ...
    def hex_get_obs(self, heading_spoof_angle=0):
        ...
        roll, pitch, yaw, quat, timestamp = self.Ahrs.update(heading_spoof_angle=heading_spoof_angle)
        ...
        t_obs = T.tensor(obs.astype(np.float32)).unsqueeze(0)
    ...
    def test_AHRS_RS(self):
        while True:
            roll, pitch, yaw_corrected, quat_yaw_corrected, self.timestamp = self.Ahrs.update()
